...
def publish(folder: Path, run_id: str, attempt: str):
    ...
    expected_url = 'https://github.com/' + os.environ['GITHUB_REPOSITORY'] + '/actions/runs/' + run_id
    ...
    entries.append({'path': 'pokemon/results/README.md', 'mode': '100644', 'type': 'blob', 'content': latest})
    for attempt_index in range(3):
        ...
        commit = api('git/commits', {'message': f'docs(pokemon): publish replay for run {run_id} [skip ci]',
        ...
        try:
            ...
            if exc.code not in (409, 422) or attempt_index == 2:
                raise
            time.sleep(1)
    url = 'https://github.com/' + os.environ['GITHUB_REPOSITORY'] + '/blob/main/' + prefix + '/README.md'
