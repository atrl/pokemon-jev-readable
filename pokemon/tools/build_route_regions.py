"""Offline conversion from source tiles/portals to conservative region topology."""
...
from route_regions import DIRECTIONS, _region
...
def build(source: Path) -> dict:
    ...
    constants = {
        ...
        for i, name in enumerate(
            re.findall(r"^\s*const\s+(\w+)", (source / "constants/tilesets.asm").read_text(), re.M)
        )
    }
    tilesets = []
    for match in re.finditer(
        ...
        tilesets.append(
            (binaries[args[0]], binaries[args[2]], {_number(value) for value in args[3:6]} - {255})
        )
    collision_source = (source / "home/overworld.asm").read_text()
    pairs_text = collision_source.split("TilePairCollisionsLand::", 1)[1].split(
        "TilePairCollisionsWater::", 1
    )[0]
    pairs = {}
    for tileset, a, b in re.findall(
        r"db\s+(\w+),\s*(\$[\dA-Fa-f]+),\s*(\$[\dA-Fa-f]+)", pairs_text
    ):
        pairs.setdefault(constants[tileset], set()).add(frozenset((_number(a), _number(b))))
    ledges = []
    for direction, a, b in re.findall(
        r"db\s+SPRITE_FACING_(\w+),\s*(\$[\dA-Fa-f]+),\s*(\$[\dA-Fa-f]+)",
    ...
    rule_files = (
        ...
        "constants/tilesets.asm",
        "macros/data_macros.asm",
        "home/overworld.asm",
    ...
    for mid in sorted(EARLY_MAP_IDS):
        ...
        if not header_path:
            skipped[str(mid)] = "missing_source_header"
            continue
        ...
        if len(blocks) != width * height // 4:
            skipped[str(mid)] = "block_count_does_not_match_dimensions"
            continue
        # The engine samples each walking tile's lower-left 8px tile.
        tiles = [
            [
                blockset[
                    blocks[(y // 2) * (width // 2) + x // 2] * 16
                    + (2 * (y % 2) + 1) * 4
                    + 2 * (x % 2)
                ]
                ...
                while queue:
                    ...
                    for dx, dy in DIRECTIONS.values():
                        ...
                        if not (0 <= nx < width and 0 <= ny < height) or rows[ny][nx] >= 0:
                            continue
                        if (
                            tiles[ny][nx] not in collision
                            or frozenset((tiles[py][px], tiles[ny][nx])) in forbidden
                        ):
                            continue
                        rows[ny][nx] = count
                        queue.append((nx, ny))
        ...
        entry = {
            ...
            "region_rows": rows,
            "counter_cells": [
                {"x": x, "y": y, "tile_id": tiles[y][x], "quality": "source_prior"}
                for y in range(height)
                for x in range(width)
                if tiles[y][x] in counter_tiles
            ],
            "warps": deepcopy(prior["warps"]),
            "connections": [],
        ...
        for match in re.finditer(r"(NORTH|SOUTH|WEST|EAST)_MAP_CONNECTION\s+([^;\n]+)", header):
            ...
            entry["connections"].append(
                {**original, "coordinate_alignment": -2 * (_number(args[2]) - _number(args[3]))}
            )
        maps[str(mid)] = entry
        tiles_by_map[mid] = tiles
        ...
        edges.append(
            {
                ...
                "transition": {
                    **transition,
                    ...
                    "destination_name": maps[str(destination)]["name"],
                    "arrival": [ax, ay],
                    "quality": "source_prior",
                    "source": "pinned_source_land_regions_and_portals",
                },
            }
        )
    ...
    for key, entry in maps.items():
        ...
        for warp in entry["warps"]:
            ...
            if destination == -1:
                ...
                parents = [
                    int(k)
                    for k, prior in world.items()
                    if int(k) <= 36 and any(w["destination_map_id"] == mid for w in prior["warps"])
                ]
                if len(parents) != 1:
                    continue
        ...
        for link in entry["connections"]:
            destination = link["destination_map_id"]
            other = maps.get(str(destination))
            if not other:
                continue
            direction = link["direction"]
            offset = link["coordinate_alignment"]
            if direction in ("north", "south"):
                points = [
                    (
                        x,
                        0 if direction == "north" else entry["height"] - 1,
                        x + offset,
                        other["height"] - 1 if direction == "north" else 0,
                    )
                ...
                points = [
                    (
                        0 if direction == "west" else entry["width"] - 1,
                        y,
                        other["width"] - 1 if direction == "west" else 0,
                        y + offset,
                    )
                ...
                for x in range(entry["width"]):
                    for direction, start_tile, ledge_tile in ledges:
                        ...
                        if (
                            ...
                            and 0 <= nx < entry["width"]
                            and 0 <= ny < entry["height"]
    ...
    return {
        ...
        "quality": "source_prior",
        "generation": "lower-left walking tiles; passable collision list; tileset-header counters; forbidden tile pairs; directional ledges; exact warp indices and connection coordinate alignment",
