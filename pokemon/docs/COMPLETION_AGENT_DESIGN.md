...
当前采用第二条路径为主：程序提供任务图、导航建议和战斗资源策略，JEV 选择全部实际按键；完整主线仍需要后续逐项验证后期事件、机关与队伍策略，不能从开场阶段通过推断已通关。

在第二条路径之上补充了**按需的高层规划**：`planning.py` 用确定性信号（连续无新坐标、位置循环、短窗口反复回城）判断是否卡住，命中且配置了 `DEEPSEEK_API_KEY` 时调用 DeepSeek 输出一个结构化建议（子目标、目标地图/对象、野外战斗资源策略、到期步数）。建议写入 campaign 记忆并进入 `current_focus`，JEV 仍逐键决定，未配置或调用失败时自动退化为纯本地策略。野外战斗默认偏向逃跑、必要时用物品捕获，以压缩洞穴遇敌耗时。规划输出只降低搜索空间，不构成完成证据。
