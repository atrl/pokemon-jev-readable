...
def verify(rom: Path, state: Path, output: Path) -> dict:
    ...
    report = {
        ...
        'rom_sha1': profile['rom_sha1'], 'source_commit': profile['source_commit'],
        'source_binary_match': False, 'input_state_sha256': EXPECTED_STATE_SHA256,
        'policy': 'isolated_copy_scripted_regression_not_jev',
        'jev_calls': 0, 'production_actions': 0, 'screenshots_created': 0,
        'checks': {},
        'source_evidence': {
    ...
    with tempfile.TemporaryDirectory(prefix='jev-observation-regression-') as folder:
        ...
        try:
            ...
            report['checks']['blank_overworld'] = {
                ...
                'scene': initial['scene'], 'dialog': initial['dialog'], 'local_map': grid,
                'collision_table_rom0_address': '0x1744', 'collision_table_bytes': table.hex(),
            }
            ...
            for button, dx, dy in [('right', 1, 0), ('left', -1, 0), ('down', 0, 1), ('up', 0, -1)]:
                ...
                movements.append({'button': button, 'before': [before['x'], before['y']],
                                  'after': [after['x'], after['y']], 'facing': after['facing'],
                                  'raw_facing': reader.byte('PlayerFacingDirection'), 'moved': moved,
            ...
            report['checks']['dialog_close_then_a_reopens'] = {
                'closed_without_moving': True, 'dialog_pages': pages,
                'reopened_same_first_page': True, 'reopened_dialog': reopened['dialog'],
            }
            ...
            report['checks']['main_menu_cursor'] = {
                'scene': menu_before['scene']['mode'], 'before': menu_before['menu_cursor_raw'],
                'after': menu_after['menu_cursor_raw'], 'visible_selection': 'PACK', 'b_returns_to_overworld': True,
            }
            report['status'] = 'passed'
            report['not_tested'] = ['Other maps', 'NPC collision', 'Warps/exits', 'Ledges', 'Battles',
                                    'Story progress', 'Jev behavior after prompt changes']
        finally:
            world.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output', type=Path, default=Path('pokemon/evidence/observation-verified.json'))
    args = parser.parse_args()
    result = verify(args.rom, args.state, args.output)
    print(json.dumps({'status': result['status'], 'checks': list(result['checks']),
