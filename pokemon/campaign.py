...
class CampaignPlanner:
    def __init__(self, snapshot=None, selector=None):
        ...
        if data.get("version", 1) != 1:
            raise ValueError("Unsupported campaign memory version")
    ...
    def _observe(self, observation):
        ...
        if (
            (observation.get("scene") or {}).get("mode") == "overworld"
            and current.get("verified")
            and world.get("source_match", True)
            and world.get("player_position_valid", True)
        ):
            ...
            for gy, row in enumerate(current.get("rows", [])):
                for gx, tile in enumerate(row):
                    ...
                    if (
                        tx < 0
                        or ty < 0
                        or (type(width) is int and tx >= width)
                        or (type(height) is int and ty >= height)
                    ):
                        continue
                    if tile in ".#@":
                        cells[cell_key(tx, ty)] = {
                            "passable": tile != "#",
                            "source": "observed_background",
                        }
        ...
        for name, fact in (observation.get("milestones") or {}).items():
            ...
            if (
                ...
                and not name.startswith("elite4_")
                and name not in ("champion_defeated", "party_fully_healed")
            ):
        ...
        if dialog.get("open") is True and dialog.get("text"):
            ...
            if fingerprint not in self.seen_clues:
                self.seen_clues.append(fingerprint)
                self.seen_clues = self.seen_clues[-4096:]
                self.clues.append(
                    {
                        "id": fingerprint,
                        "map_id": map_id,
    ...
    def record(self, button, before, after):
        before_position, after_position = position(before), position(after)
        self.steps += 1
        self._observe(before)
        new_clue = self._observe(after)
        if after_position:
            key = f"{after_position[0]}:{after_position[1]},{after_position[2]}"
            self.visits[key] = self.visits.get(key, 0) + 1
            self._last_position = list(after_position)
        if before_position and after_position and before_position[0] != after_position[0]:
            control = (before.get("world") or {}).get("input_lock") or {}
            manual = (
                button in DIRECTIONS
                and (before.get("scene") or {}).get("mode") == "overworld"
                and not control.get("ignored_buttons_mask", 0)
                and not control.get("scripted_movement_remaining", 0)
            )
            edge = {
                "from_map": before_position[0],
                "from_position": [before_position[1], before_position[2]],
                "button": button,
                "to_map": after_position[0],
                "arrival": [after_position[1], after_position[2]],
                "source": "observed_map_transition",
                "step": self.steps,
        ...
        if (
            ...
            key = f"{before_position[0]}:{before_position[1]},{before_position[2]}:{button}"
            lock = (before.get("world") or {}).get("input_lock") or {}
            facing_changed = (before.get("player") or {}).get("facing") != (
                after.get("player") or {}
            ).get("facing")
            ...
            if (
                ...
                and not lock.get("ignored_buttons_mask", 0)
            ):
                self.failed_edges[key] = self.failed_edges.get(key, 0) + 1
                self.failed_edge_steps[key] = self.steps
            elif before_position != after_position:
                self.failed_edges.pop(key, None)
                self.failed_edge_steps.pop(key, None)
        # Moving NPCs and scripted locks must not poison before_position route for the rest
    ...
    def _target_for(self, observation, objective):
        ...
        if p is None:
            return None, []
        mid, x, y = p
        target_map = objective.get("target_map_id")
        ...
        routing = plan_route(
            ...
            target_map,
            target=selectors,
            facts=observation.get("milestones"),
        )
        route = routing.get("map_route") or []
        if routing.get("status") == "planned":
            return {
                **routing["next_transition"],
                "route_quality": routing["quality"],
                "route_limitations": routing.get("limitations"),
            }, route
        if routing.get("status") != "same_region":
            return {
                ...
                "quality": routing.get("quality"),
                "limitations": routing.get("limitations"),
            }, route
        targets = objective.get("interaction_selectors") or (
            [objective.get("target")] if objective.get("target") else []
        )
        choices = []
        for target in targets:
            ...
            if target.get("kind") == "object":
                matches = [
                    ...
                    if obj.get("active") is not False and _matches_object(obj, target)
                ]
                for obj in matches:
                    choices.append(
                        {
                            ...
                            **deepcopy(obj),
                            "kind": "object",
                            "object_category": obj.get("kind"),
                        }
                    )
                hidden = any(
                    obj.get("active") is False and _matches_object(obj, target)
                    for obj in world.get("objects", [])
                )
                if not matches and not hidden and type(target.get("x")) is int:
                    choices.append(deepcopy(target))
            else:
                choices.append(deepcopy(target))
        if not choices:
            return None, route or [mid]
        return min(
            choices, key=lambda t: abs(t.get("x", x) - x) + abs(t.get("y", y) - y)
        ), route or [mid]
    ...
    def _navigation(self, observation, target):
        ...
        if p is None or not target:
            return {"status": "needs_target", "next_button": None}
        mid, x, y = p
        kind = target.get("kind")
        width = world.get("width")
        height = world.get("height")
        if kind == "unknown_route":
            return {"status": "needs_map_connection", "next_button": None, "target": target}
        phase = (observation.get("scene") or {}).get("mode")
        if phase != "overworld":
            return {"status": "resolve_current_ui", "next_button": None, "target": target}
        lock = world.get("input_lock") or {}
        if (
            lock.get("ignored_buttons_mask") == 255
            or lock.get("scripted_movement_remaining", 0) > 0
        ):
            return {
                "status": "script_controls_player",
                "next_button": "wait",
                "target": target,
                "source": "live game input lock and scripted movement",
            }
        if type(width) is not int or type(height) is not int:
            return {"status": "needs_geometry", "next_button": None}
        goals = set()
        activation = None
        approach_facings = {}
        if kind == "connection":
            direction = COMPASS.get(target.get("direction"))
            activation = direction
            if type(target.get("x")) is int and type(target.get("y")) is int:
                goals = {(target["x"], target["y"])}
            elif direction == "up":
                goals = {(cx, 0) for cx in range(width)}
            elif direction == "down":
                goals = {(cx, height - 1) for cx in range(width)}
            elif direction == "left":
                goals = {(0, cy) for cy in range(height)}
            elif direction == "right":
                goals = {(width - 1, cy) for cy in range(height)}
        else:
            tx, ty = target.get("x"), target.get("y")
            if type(tx) is not int or type(ty) is not int:
                return {"status": "needs_target_coordinate", "target": target, "next_button": None}
            if kind == "object":
                approaches = interaction_positions(mid, target)
                goals = {(row["x"], row["y"]) for row in approaches} or {
                    (tx + dx, ty + dy) for dx, dy in DIRECTIONS.values()
                }
            ...
            if kind == "warp":
                if ty == 0:
                    ...
                    activation = "right"
            elif kind in ("observed_transition", "ledge"):
                activation = target.get("button")
        cells = self.tiles.get(str(mid), {})
        blocked = {
            (o["x"], o["y"])
            for o in world.get("objects", [])
            if o.get("active") is True and type(o.get("x")) is int and type(o.get("y")) is int
        }
        if kind == "object":
            blocked.add((target["x"], target["y"]))
        start = (x, y)
        if start in goals:
            if kind == "object":
                delta = (target["x"] - x, target["y"] - y)
                facing = approach_facings.get(start) or next(
                    (d for d, v in DIRECTIONS.items() if v == delta), None
                )
                expected = (
                    "a" if (observation.get("player") or {}).get("facing") == facing else facing
                )
                return {
                    "status": "interact" if expected == "a" else "face_object",
                    "next_button": expected,
                    "target": target,
                    "source": "target geometry, source-labelled counter interaction cells and observed facing",
                }
            if activation:
                return {
                    "status": "activate_transition",
                    "next_button": activation,
                    "target": target,
                    "source": "standing on observed/source-labelled map boundary",
                }
            if kind == "warp":
                for direction, (dx, dy) in DIRECTIONS.items():
                    adjacent = (x + dx, y + dy)
                    if adjacent not in blocked and cells.get(cell_key(*adjacent), {}).get(
                        "passable"
                    ):
                        return {
                            "status": "reenter_warp",
            ...
            return {
                ...
                "next_button": "wait",
                "target": target,
                "source": "coordinate/script-trigger target reached; observe its effect",
            }
        return self._path_to_target(mid, start, goals, blocked, world, target)

    def _path_to_target(self, map_id, start, goals, blocked, world, target):
        """Search observed cells, falling back to a nearby unexplored frontier."""
        x, y = start
        width, height = world["width"], world["height"]
        cells = self.tiles.get(str(map_id), {})
        queue = deque([start])
        parents = {start: None}
        moves = {}
        found = None
        while queue:
            ...
            for direction, (dx, dy) in DIRECTIONS.items():
                ...
                if (
                    ...
                    continue
                if not cells.get(cell_key(*nxt), {}).get("passable"):
                    continue
                if self.failed_edges.get(f"{map_id}:{at[0]},{at[1]}:{direction}", 0) >= 2:
                    continue
                parents[nxt] = at
                moves[nxt] = direction
                queue.append(nxt)
        ...
        if found is None:
            ...
            if not reachable:
                return {
                    "status": "no_observed_path",
                    "next_button": None,
                    "target": target,
                    "source": "observed background BFS; request re-observation/replan",
                }

            def distance(c):
                return min(abs(c[0] - g[0]) + abs(c[1] - g[1]) for g in goals) if goals else 0

            frontier = [
                c
                for c in reachable
                if any(
                    cell_key(c[0] + dx, c[1] + dy) not in cells
                    and 0 <= c[0] + dx < width
                    and 0 <= c[1] + dy < height
        ...
        while parents[node] is not None:
            path.append({"x": node[0], "y": node[1], "button": moves[node]})
            node = parents[node]
        path.reverse()
        return {
            "status": status,
            "next_button": path[0]["button"] if path else None,
            "next_waypoint": path[0] if path else None,
            "path_preview": path[:8],
            "target": target,
            "source": "deterministic BFS guidance over observed background; JEV independently selects the actual input",
    ...
    def recover(self, observation, *, attempt, reason, failed_button):
        ...
        self.recovery = {
            ...
            "position": list(p) if p else None,
            "phase": (observation.get("scene") or {}).get("mode"),
    ...
    def _select_objective(self, observation, world):
        ...
        if objective.get("id") != "main_story_complete":
            ...
            if self.support:
                ...
                objective = {
                    **self.support,
                    "story_objective_id": story_objective["id"],
                    "completed_ids": story_objective.get("completed_ids", []),
                    "completion_evidence": {
                        "party_fully_healed": {
                            "value": full_recovery,
                            "verified": full_recovery is not None,
                            "quality": "verified" if full_recovery is not None else "needs_data",
                            "source": "current valid party HP, status and actual ROM-verified maximum PP",
                        }
                    },
                }
        return objective, story_objective
            ...
            self.objective_history.append(
                {
                    ...
                    "from": previous,
                    "to": objective.get("id"),
                    "reason": "verified facts evaluated against story prerequisites",
                }
            )
    ...
    def context(self, observation):
        """Refresh memory, select an objective, then derive navigation guidance."""
        self._observe(observation)
        if self.recovery and (
            self.steps > self.recovery["expires_after_action"]
            or self.recovery.get("position")
            != (list(position(observation)) if position(observation) else None)
        ...
        return {
            ...
            "route_map_ids": route,
            "route_map_names": [(map_prior(mid) or {}).get("name", str(mid)) for mid in route],
        ...
        return deepcopy(
            {
                ...
                "clues": self.clues,
                "seen_clues": self.seen_clues,
                "history_facts": self.history_facts,
                "active": self.active,
