...
OBSERVATION = {'game': 'OFFLINE TEST DOUBLE', 'errors': [], 'player': {},
               'party': [], 'bag': [], 'screen_text': {'rows': ['OFFLINE TEST DOUBLE']}}
...
class LongRunTests(unittest.TestCase):
    def test_5000_step_budget_is_accepted_without_a_key_or_network_call(self):
        with tempfile.TemporaryDirectory() as directory, \
             ...
             patch('run.Emulator') as emulator, patch('run.choose') as choose:
            output = Path(directory) / 'offline-run'
            report = run(Path('NO-ROM.gb'), output, goal='Offline validation', steps=5000,
    ...
    def test_checkpoints_are_available_during_run_with_verified_hash_and_step(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            def choose(_before, _goal, _history, *, on_event):
                ...
                on_event({'type': 'jev_response', 'attempt': 1, 'latency_ms': 100, 'httpStatus': 503})
                on_event({'type': 'jev_error', 'attempt': 1, 'latency_ms': 100, 'httpStatus': 503})
                on_event({'type': 'jev_request', 'attempt': 2})
                on_event({'type': 'jev_response', 'attempt': 2, 'latency_ms': 200, 'httpStatus': 200})
                return {'answer': {'choice': 'a'}, 'source': 'offline-test-double', 'latency_ms': 300}
            ...
            with patch.dict(os.environ, {'TYPESAFE_API_KEY': TEST_KEY}), \
                 ...
                 patch('run.load_profile', return_value=PROFILE), patch('run.choose', side_effect=choose):
                report = run(Path('NO-ROM.gb'), output, goal='Offline validation', steps=5, checkpoint_every=2)
            self.assertEqual(observed_checkpoints, [2, 4])
            self.assertEqual(saved_steps, [2, 4, 5])
            self.assertEqual([row['saved_step'] for row in events(output) if row['type'] == 'checkpoint'], [2, 4])
            manifest = json.loads((output / 'last.state.json').read_text())
...
'''
        with tempfile.TemporaryDirectory() as directory:
            ...
            process = subprocess.Popen([sys.executable, '-c', script, str(POKEMON), str(ready),
                                        '--output', str(output), '--steps', '5000', '--no-video'],
            ...
            try:
                ...
                self.assertEqual(manifest['state_sha256'], hashlib.sha256(b'offline interrupted state').hexdigest())
            finally:
