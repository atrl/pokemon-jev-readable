...
def move(move_id, slot, defender_types, pp=20):
    ...
    return {'slot': slot, 'move_id': move_id, 'pp': pp, 'knowledge': knowledge,
...
def battle(defender_types=(4,), defense=14, special=13, hp=40):
    return {'active': True, 'verified': True, 'type': 'trainer', 'menu': 'move',
            'selected_move_slot': 0, 'selected_command': None,
            'player': {'hp': 25, 'max_hp': 30, 'level': 12, 'attack': 20, 'defense': 18, 'special': 20,
                       'verified_stats': True, 'types_verified': True, 'types': [{'id': 20}],
                       'moves': [move(10, 0, defender_types), move(45, 1, defender_types), move(52, 2, defender_types)]},
            'enemy': {'hp': hp, 'max_hp': hp, 'level': 12, 'attack': 18, 'defense': defense, 'special': special,
                      'status_bits': 0, 'verified_stats': True, 'types_verified': True,
...
class BattleStrategyTests(unittest.TestCase):
    ...
    def test_geodude_resists_both_but_low_special_still_favors_ember(self):
        ...
        self.assertEqual(damage['EMBER']['type_effectiveness'], .5)
        self.assertEqual(damage['SCRATCH']['type_effectiveness'], .5)
    ...
    def test_empty_pp_and_status_moves_are_excluded(self):
        state = battle(); state['player']['moves'][2]['pp'] = 0
        result = plan_battle(state)
    ...
    def test_type_immunity_never_receives_positive_damage_estimate(self):
        state = battle((8,)); state['player']['moves'] = [state['player']['moves'][0]]
        result = plan_battle(state)
        self.assertIsNone(result['recommended_slot'])
    ...
    def test_unknown_move_numbers_chart_stats_or_attacker_type_do_not_invent_neutral_damage(self):
        for missing in ('numbers', 'chart', 'stats', 'types'):
            state = battle(); state['player']['moves'] = [state['player']['moves'][2]]
            if missing == 'numbers': state['player']['moves'][0]['knowledge']['numeric_data_verified'] = False
            elif missing == 'chart': state['player']['moves'][0]['effectiveness']['verified'] = False
            elif missing == 'stats': state['enemy']['verified_stats'] = False
            elif missing == 'types': state['player']['types_verified'] = False
            result = plan_battle(state)
    ...
    def test_normalized_cursor_changes_one_step_and_does_not_read_raw_cursor(self):
        for selected, expected in ((0, 'down'), (1, 'down'), (2, 'a')):
            state = battle(); state['selected_move_slot'] = selected
            self.assertEqual(plan_battle(state)['next_button'], expected)
        state['selected_move_slot'] = None; state['menu_cursor_raw'] = 3
        result = plan_battle(state)
        self.assertEqual(result['recommended_slot'], 2)
        self.assertIsNone(result['next_button'])

    def test_visible_command_labels_navigate_toward_fight_without_raw_index_assumptions(self):
        for command, expected in (('FIGHT', 'a'), ('PKMN', 'left'), ('ITEM', 'up'), ('PACK', 'up'), ('RUN', 'up'), (None, None)):
            state = battle(); state.update(menu='command', selected_command=command, menu_cursor_raw=0)
    ...
    def test_fainted_combatants_do_not_request_another_attack(self):
        for side in ('player', 'enemy'):
            state = battle(); state[side]['hp'] = 0
            result = plan_battle(state)
            self.assertIsNone(result['recommended_slot'])
            self.assertIsNone(result['next_button'])

    def test_conditional_hit_requirement_is_checked_and_unknown_is_not_assumed(self):
        state = battle(); state['player']['moves'] = [move(138, 0, [4])]
        self.assertEqual(plan_battle(state)['ranked_moves'][0]['exclusion_reason'], 'dream_eater_requires_sleeping_target')
    ...
    def test_noncritical_integer_formula_stat_scaling_and_type_order(self):
        ...
        base, ordered, *_ = _damage_distribution(10, 30, 20, 20, False, [.5, 2])
    ...
    def test_hit_probability_includes_gen_one_255_over_256_limit(self):
        result = plan_battle(battle())
        self.assertAlmostEqual(result['ranked_moves'][0]['base_hit_probability'], 255 / 256, places=6)

    def test_disabled_and_complex_mechanics_are_not_ranked_as_simple_free_damage(self):
        state = battle(); state['player']['moves'] = [move(120, 0, [4])]
        self.assertEqual(plan_battle(state)['ranked_moves'][0]['exclusion_reason'], 'conditional_mechanic_not_estimated')
        state = battle(); state['player']['moves'][2]['disabled'] = True
        self.assertEqual(plan_battle(state)['recommended_slot'], 0)

    def test_advice_is_pure_and_only_input_request_is_an_advisory_value(self):
        state = battle(); original = deepcopy(state)
