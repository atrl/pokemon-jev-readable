...
def state(x=4,text='Challenge!',mode='battle'):
    return {'player':{'map_id':54,'x':x,'y':6},'world':{'source_match':True,'player_position_valid':True},
            'scene':{'mode':mode,'verified':True},'screen_text':{'rows':[text]},
            'dialog':{'text':text,'open':mode=='dialog'},
            'battle':{'active':mode=='battle','phase_verified':True,'phase':'intro','menu':'text_or_animation','visible_text':text}}


class ActivityTests(unittest.TestCase):
    def test_exact_defeat_return_and_rematch_trace_does_not_stop_at_101(self):
        fixture=json.loads((Path(__file__).parent/'fixtures/defeat_return_and_rematch.json').read_text())
        monitor=StallMonitor(80,0)
        for row in fixture['rows']:
            activity=monitor.observe(row['before'],row['after'],row['outcome'],row['progress'])
            self.assertFalse(activity['exhausted'],f"Unexpected stop at former production step {row['step']}")
    ...
    def test_known_route_movement_is_activity_without_claiming_story_progress(self):
        ...
        for x in range(30):
            result=monitor.observe(state(x,mode='overworld'),state(x+1,mode='overworld'),{}, {'loop_kind':None})
    ...
    def test_cycling_positions_still_has_bounded_recovery(self):
        ...
        for i in range(12):
            result=monitor.observe(state(i%2,mode='overworld'),state((i+1)%2,mode='overworld'),{}, {'loop_kind':'position_cycle'})
        self.assertTrue(result['should_recover']);self.assertFalse(result['exhausted'])
    ...
    def test_stable_game_waits_recover_then_pause_but_no_move_is_selected(self):
        monitor=StallMonitor(12,3);s=state()
        for attempt in range(3):
            for _ in range(12):result=monitor.observe(s,s,{}, {'loop_kind':'stationary_repetition'})
            self.assertTrue(result['should_recover']);self.assertEqual(monitor.begin_recovery(),attempt+1)
        for _ in range(12):result=monitor.observe(s,s,{}, {'loop_kind':'stationary_repetition'})
        self.assertTrue(result['exhausted']);self.assertNotIn('next_button',result)
