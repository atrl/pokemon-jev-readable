...
def world_state(x=3, mode='overworld', verified=True):
    return {'game':'OFFLINE TEST DOUBLE','errors':[],
            'player':{'map_id':38,'x':x,'y':6,'name':'TEST','facing':'up','facing_quality':'verified_direction_response'},
            'scene':{'mode':mode,'verified':verified,'source':'test fixture'},
            'dialog':{'open':mode=='dialog','awaiting_input':mode=='dialog','text':'TEST' if mode=='dialog' else ''},
            'screen_text':{'rows':['']*18},'party':[],'bag':[],
            'local_map':{'verified':verified,'quality':'advisory_background_only',
                         'neighbors':{'up':{'background_passable':False},'right':{'background_passable':True}}}}
...
class DecisionContextTests(unittest.TestCase):
    def test_verified_blank_world_reaches_model_with_geometry_and_all_buttons(self):
        before=world_state();tracker=ProgressTracker()
        for _ in range(24):tracker.record('a',before,before)
        ...
        self.assertFalse(request['questions']['button']['criteria']['up']['background_neighbor']['background_passable'])
        self.assertTrue(request['questions']['button']['criteria']['right']['background_neighbor']['background_passable'])

    def test_request_carries_three_chronological_action_aligned_states(self):
        tracker=ProgressTracker()
        for x in range(3,8):tracker.record('right',world_state(x),world_state(x+1))
        current=world_state(8);current['progress']=tracker.context(current)
    ...
    def test_background_tile_letters_do_not_become_world_dialog(self):
        raw=world_state();raw['screen_text']['rows']=['FAKE BACKGROUND LETTERS']
        model=observation_for_model(raw)
        self.assertEqual(model['screen_text']['rows'],[])
        self.assertIs(model['dialog']['open'],False)
        raw=world_state(mode='dialog');raw['screen_text']['rows']=['FAKE UPPER BACKGROUND','Actual text']
    ...
    def test_unverified_mode_geometry_and_facing_are_not_promoted(self):
        raw=world_state(verified=False);raw['player']['facing_quality']='needs_data'
        model=observation_for_model(raw)
    ...
    def test_checkpoint_memory_requires_the_same_state_identity(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            manifest={'rom_sha1':'test-only','state_sha256':hashlib.sha256(state.read_bytes()).hexdigest(),'step':1}
            tracker=ProgressTracker();tracker.record('right',world_state(),world_state(4))
            ...
            restored,source=restore_progress(state,{**manifest,'state_sha256':'another-state'})
            self.assertEqual(source,'new_memory');self.assertEqual(restored.total_steps,0)

    def test_legacy_effect_replay_stops_at_the_saved_step(self):
        with tempfile.TemporaryDirectory() as directory:
            state=Path(directory)/'last.state'
            rows=[{'type':'result','step':step,'success':True,'button':'a',
                   'result':{'before':world_state(),'after':world_state()}} for step in range(1,25)]
            (state.parent/'events.jsonl').write_text(''.join(json.dumps(row)+'\n' for row in rows))
            tracker,source=restore_progress(state,{'rom_sha1':'test-only','state_sha256':'test-only','step':12})
            self.assertEqual(source,'legacy_observed_effects:12');self.assertEqual(tracker.total_steps,12)
    ...
    def test_verified_repetitive_run_stops_and_saves_instead_of_spending_budget(self):
        with tempfile.TemporaryDirectory() as directory:
            output=Path(directory)/'run';world=Mock();world.save.return_value=b'OFFLINE STATE'
            ...
            self.assertEqual(saved['state_sha256'],hashlib.sha256((output/'last.state').read_bytes()).hexdigest())
