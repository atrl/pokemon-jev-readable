...
def observable_state(observation):
    ...
    if battle.get("verified") is True:
        result["battle_selection"] = (
            battle.get("selected_command"),
            battle.get("selected_move_slot"),
        )
    ...
    if (
        count.get("verified") is True
        and isinstance(party, list)
        and len(party) == count.get("value")
    ):
        result["party"] = [
            (
                m.get("species_internal_id"),
                m.get("level"),
                m.get("hp"),
                m.get("status_bits"),
                [(v.get("move_id"), v.get("pp")) for v in m.get("moves", [])],
            )
            for m in party
        ]
...
class StallMonitor:
    def __init__(self, threshold=80, max_recovery_attempts=3):
        ...
        self.no_effect_steps = 0
        self.no_strategic_progress_steps = 0
    ...
    def observe(self, before, after, outcome, progress):
        ...
        self.no_strategic_progress_steps = (
            0 if outcome.get("meaningful_progress") else self.no_strategic_progress_steps + 1
        )
        cycle = progress.get("loop_kind") in (
            "position_cycle",
        ...
        if (
            ...
            or outcome.get("milestones_completed")
            or outcome.get("battle_damage")
            or self.active_steps >= 12
        ):
            self.recovery_attempts = 0
        reason = (
            ...
            if cycle and self.no_strategic_progress_steps >= self.threshold
            else None
        )
        return {
            ...
            "should_recover": reason is not None
