...
SPECIAL_DAMAGE_EFFECTS = {"SPECIAL_DAMAGE_EFFECT", "BIDE_EFFECT", "SUPER_FANG_EFFECT", "OHKO_EFFECT"}
...
def _verified(record):
    ...
    if "unverified" in quality or "prior" in quality:
        return False
    return record.get("verified") is True or quality == "verified" or quality.startswith("verified_")
...
def _stable(snapshot):
    ...
    return (scene.get("verified") is True and scene.get("mode") == "overworld"
            and _verified(battle) and battle.get("active") is False
...
def _max_pp(move):
    ...
    if isinstance(raw, dict):
        value = _value(raw)
    elif _verified({"verified": move.get("max_pp_verified"), "quality": move.get("max_pp_quality", "")}):
        value = raw
    else:
        return None
...
def assess_recovery(snapshot):
    ...
    result = {"party_verified": False, "needs_recovery": None, "full_recovery": None,
              "trigger_reasons": [], "missing_evidence": [], "stable_overworld": _stable(snapshot),
              "hp_ratio": None, "viable_count": None, "damage_pp_exhausted": None,
              "members": [], "damage_classification_quality": []}
    if (not isinstance(party, list) or not _integer(count, 1, 6) or count != len(party)
            or party_state.get("ready") is not True or not _verified(party_state)):
        result["missing_evidence"].append("verified_ready_party")
        return result
    moves = []
    for index, mon in enumerate(party):
        if not isinstance(mon, dict) or not (_integer(mon.get("hp"), 0, 999)
                and _integer(mon.get("max_hp"), 1, 999) and mon["hp"] <= mon["max_hp"]
                and _integer(mon.get("status_bits"), 0, 255)):
            result["missing_evidence"].append(f"party[{index}].hp_or_status")
            return result
        row = {"slot": mon.get("slot", index), "hp": mon["hp"], "max_hp": mon["max_hp"],
               "status_bits": mon["status_bits"], "moves": []}
        ...
        for move_index, move in enumerate(mon_moves):
            if not isinstance(move, dict) or not (_integer(move.get("move_id"), 1, 255)
                                                                 and _integer(move.get("pp"), 0, 63)):
                result["missing_evidence"].append(f"party[{index}].moves[{move_index}].pp")
                continue
            maximum = _max_pp(move)
            damage, quality = _damaging(move)
            entry = {"move_id": move["move_id"], "pp": move["pp"], "max_pp": maximum,
                     "damaging": damage, "classification_quality": quality}
            row["moves"].append(entry)
            if maximum is None:
                result["missing_evidence"].append(f"party[{index}].moves[{move_index}].verified_max_pp")
            elif move["pp"] > maximum:
                result["missing_evidence"].append(f"party[{index}].moves[{move_index}].incoherent_pp")
            if mon["hp"] > 0:
                moves.append(entry)
    ...
    if not viable:
        ...
        reasons.append("last_viable_member_low_hp" if len(viable) == 1 else "combined_viable_hp_low")
    ...
    pp_missing = any(".moves" in key and not key.endswith(".verified_max_pp") for key in result["missing_evidence"])
    ...
    result["damage_classification_quality"] = sorted({move["classification_quality"] for move in moves})
    result["needs_recovery"] = bool(reasons)
    hp_full = all(mon["hp"] == mon["max_hp"] for mon in result["members"])
    clear_status = all(mon["status_bits"] == 0 for mon in result["members"])
    all_moves = [move for mon in result["members"] for move in mon["moves"]]
    known_short_pp = any(move["max_pp"] is not None and move["pp"] < move["max_pp"] for move in all_moves)
    current_healed = facts.get("party_fully_healed") or {}
    fresh_marker = (_value(current_healed) is True and type(snapshot.get("frame")) is int
                    and current_healed.get("frame") == snapshot["frame"])
    if not hp_full or not clear_status or known_short_pp or any(move["pp"] == 0 for move in all_moves):
        result["full_recovery"] = False
    elif not result["missing_evidence"]:
        result["full_recovery"] = True
...
def _route_to(snapshot, map_id, target, router):
    result = router(snapshot.get("world") or {}, snapshot.get("player") or {},
                    map_id, target=target, facts=snapshot.get("milestones") or {})
    return result if isinstance(result, dict) else {"status": "needs_data", "reason": "invalid_region_route_response"}
...
def _clinic_target(snapshot, data, router):
    ...
    for key, record in data.get("maps", {}).items():
        if not str(key).isdigit() or not isinstance(record, dict):
            continue
        map_id = int(key)
        if "POKECENTER" not in str(record.get("name", "")) and record.get("name") != "INDIGO_PLATEAU_LOBBY":
            continue
        nurse = next((obj for obj in record.get("objects", []) if obj.get("sprite") == "SPRITE_NURSE"
                      and type(obj.get("x")) is int and type(obj.get("y")) is int), None)
        if nurse is None:
            continue
        target = {"kind": "object", "sprite": "SPRITE_NURSE", "x": nurse["x"], "y": nurse["y"],
                  "text_id": nurse.get("text_id"), "quality": "source_prior"}
        route = _route_to(snapshot, map_id, target, router)
        diagnostics.append({"target_map_id": map_id, "status": route.get("status", "needs_data"),
                            "reason": route.get("reason")})
        if route.get("status") not in ("planned", "same_region"):
            continue
        hops = max(0, len(route.get("region_route") or route.get("map_route") or []) - 1)
        candidate = {"target_map_id": map_id, "target_map_name": record.get("name"),
                     "route_map_ids": deepcopy(route.get("map_route", [])), "target": target,
    ...
    if candidates:
        ...
        return candidate
    return {"target_map_id": None, "target": None, "route_map_ids": [],
            "routing": {"status": "needs_data" if not diagnostics or any(row["status"] != "unreachable" for row in diagnostics) else "unreachable",
                        "reason": "no_supported_reachable_clinic", "clinic_candidates": diagnostics,
...
def plan_support(snapshot, active=None, *, world_data=None, route_planner=None):
    ...
    if active:
        if assessment["full_recovery"] is True and assessment["stable_overworld"]:
            return None
    elif not (assessment["party_verified"] and assessment["needs_recovery"] is True and assessment["stable_overworld"]):
        return None
    objective = deepcopy(active) if active else {}
    router = route_planner or plan_route
    if assessment["stable_overworld"]:
        if objective.get("target"):
            routing = _route_to(snapshot, objective.get("target_map_id"), objective["target"], router)
            objective["routing"] = deepcopy(routing)
            if routing.get("status") == "unreachable":
                rejected = objective.setdefault("rejected_targets", [])
                rejected.append({"target_map_id": objective.get("target_map_id"),
                                 "target": deepcopy(objective["target"]), "reason": routing.get("reason", "region_route_unreachable"),
                                 "route_status": "unreachable", "player": deepcopy(snapshot.get("player")),
        ...
        objective["routing"] = {"status": "needs_data", "reason": "await_stable_overworld_before_route_check"}
    reason_labels = {
    ...
    objective.update({
        "id": "heal_party", "intent": "到宝可梦中心回复全队 HP、PP 并清除异常状态",
        "why": objective.get("reason", "继续完成已开始的队伍恢复任务"),
        "status": "needs_recovery", "completion": False,
        "needs_recovery": assessment["full_recovery"] is not True,
        "ready_to_navigate": assessment["stable_overworld"] and not target_missing and route_ready,
        "route_status": routing.get("status", "needs_data"),
        "target_map_id": objective.get("target_map_id"), "target": target,
        "interaction_selectors": [deepcopy(target)] if target else [],
        "evidence": assessment, "unknown_facts": assessment["missing_evidence"] + (["reachable_pokecenter"] if target_missing else [])
        + (["clinic_region_route"] if not route_ready else []),
        "knowledge": ["按当前护士对话完成恢复，随后重新验证全队 HP、每个招式 PP 和状态。",
                      "本目标保持到恢复得到验证；低等级本身不会改变主线目标。",
                      "地图路径和护士位置来自源码先验，途中需要实际观察确认。"],
        "source": {"quality": "source_prior", "method": "fewest supported source-region portals among reachable clinic interaction targets; live gates still require verification",
                   "source_binary_match": False},
    })
    return objective


support_objective = plan_support
