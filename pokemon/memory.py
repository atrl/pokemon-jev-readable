...
class Reader:
    def __init__(self, emulator, profile: dict):
        ...
        self._verified_pp_bonus = (
            rom.count(PP_BONUS_SIGNATURE) == 1
        ...
        self._party_naming_return_pc = (
            (at if at < 0x4000 else 0x4000 + at % 0x4000) if rom.count(naming_return) == 1 else None
        )
    ...
    def screen_text(self) -> dict:
        ...
        rows = [
            decode_text(tiles[i : i + 20], terminated=False, unknown=" ") for i in range(0, 360, 20)
        ]
        return {
            "rows": rows,
            "source": "RAM wTileMap, not OCR",
    ...
    def move(self, slot: int, move_id: int, raw_pp: int) -> dict:
        ...
        knowledge["numeric_data_quality"] = (
            "verified_exact_rom_move_table"
            if knowledge["numeric_data_verified"]
            else "source_prior"
        )
        base_pp = knowledge.get("base_pp")
        pp_ups = raw_pp >> 6
        verified = bool(
            self._verified_move_data and self._verified_pp_bonus and isinstance(base_pp, int)
        )
        ...
        return {
            ...
            "pp_ups": pp_ups,
            "max_pp": maximum,
            "max_pp_verified": verified,
            "max_pp_source": "Entire 165x6-byte move table and exact-ROM AddBonusPP/Divide instructions; base + min(base//5,7) * high-two-bit PP Up count"
            if verified
            else "needs_data: move table or PP Up calculation not verified against exact ROM",
            "knowledge": knowledge,
        }
    ...
    def party(self) -> list[dict]:
        ...
        for i in range(count):
            ...
            result.append(
                {
                    "slot": i,
                    "nickname": decode_text(self.data("wPartyMonNicks", 11, i * 11)),
                    "species_internal_id": mon[0],
                    "level": mon[33],
                    "hp": hp,
                    "max_hp": max_hp,
                    ...
                    "status_bits": mon[4],
                    "moves": [
                        self.move(j, mon[8 + j], mon[29 + j]) for j in range(4) if mon[8 + j]
                    ],
                }
            )
        return result
    ...
    def _pending_party_registration(self, count: int, slot: int, mon: bytes) -> dict | None:
        ...
        if (
            species[-1] != 255
            or not all(named_lookup("species", value) for value in species[:-1])
            or self.byte("wMonDataLocation") != 0
            or self.byte("wNamingScreenType") != 2
            or self.byte("hNewPartyLength") != count
            or self.byte("wcf91") != species[slot]
        ...
        return {
            "ready": False,
            "quality": "verified_interactive_initialization",
            "phase": "nickname_interaction_before_party_structure",
            "pending_slot": slot,
            "pending_species_internal_id": species[slot],
            "pending_species_name_prior": named_lookup("species", species[slot]),
            "count_raw": count,
            "registered_usable_count": None,
    ...
    def bag(self) -> list[dict]:
        ...
        for i in range(0, len(raw), 2):
            item_id = raw[i]
            verified_name = (
                1 <= item_id <= len(load_world_data()["name_tables"]["items"]["names"])
                and self._verified_name_tables["items"]
                or 196 <= item_id <= 200
                and self._verified_hm_ids
            )
            result.append(
                {
                    "item_id": item_id,
                    "quantity": raw[i + 1],
                    "name_prior": named_lookup("items", item_id),
                    "name_verified": verified_name,
                    "quality": "verified_current_bag_and_rom_name"
                    if verified_name
                    else "current_bag_source_name_prior",
                }
            )
        return result
    ...
    def world(self, scene: dict | None = None) -> dict:
            ...
            warps.append(
                {
                    ...
                    "destination_map_id_raw": destination_raw,
        ...
        for i in range(0, 34, 2):
            ...
            if not 1 <= slot <= n_sprites:
                break
            hidden[slot] = bool(
                self.data("wMissableObjectFlags", 1, flag // 8)[0] & (1 << (flag % 8))
            )
            ...
            if sprite_table_valid and usable_scene:
                ...
                if coherent and hidden_valid and hidden.get(slot, False):
                    item.update(
                        ...
                        active=False,
                        visible=False,
                        quality="verified_hidden_sprite",
                        source="RAM current map missable-object list and hidden bit",
                    )
                    ...
                    item.update(
                        ...
                        present=True,
                        active=True,
                        facing=FACING.get(state1[9]),
                        quality="verified_current_ram",
                        visible=state1[2] != 255,
                    ...
                    item.update(
                        ...
                        active=False,
                        visible=False,
                        quality="verified_absent_sprite",
                        source="RAM sprite picture ID is zero; source object may be hidden",
                    )
            objects.append(item)
        connections = []
        mask = self.byte("wMapConnections")
        for bit, direction, symbol in [
            (8, "north", "wMapConn1Ptr"),
            (4, "south", "wMapConn2Ptr"),
            (2, "west", "wMapConn3Ptr"),
            (1, "east", "wMapConn4Ptr"),
        ]:
            if not mask & bit:
            ...
            known = next(
                (
                    c
                    for c in prior["connections"]
                    if c["direction"] == direction and c["destination_map_id"] == dest
                ),
                None,
            )
        ...
        return {
            ...
            "width": width,
            "height": height,
            "player_position_valid": 0 <= self.byte("wXCoord") < width
            and 0 <= self.byte("wYCoord") < height,
            "warps": warps,
            "objects": objects,
            "signs": prior["signs"],
            ...
            "input_lock": {
                "ignored_buttons_mask": self.byte("wJoyIgnore"),
                "scripted_movement_remaining": self.byte("wSimulatedJoypadStatesIndex"),
                "quality": "source_profile_live_ram",
                "source": "wJoyIgnore and wSimulatedJoypadStatesIndex",
            },
            "quality": "verified_current_map_geometry" if matched else "needs_data",
            "source_match": matched,
            "source": {
                "commit": load_world_data()["source_commit"],
                "source_binary_match": False,
                "header": prior.get("source_header"),
                "objects": prior.get("source_objects"),
                "live_fields": "map ID, dimensions, warp tuples, connections, sprite occupancy",
            },
    ...
    def battle(self) -> dict:
        ...
        def combatant(symbol, nickname):
            ...
            stats = {
                name: int.from_bytes(mon[offset : offset + 2], "big")
                for name, offset in (
                    ("attack", 17),
                    ("defense", 19),
                    ("speed", 21),
            ...
            types = [
                {
                    "id": value,
                    "name": load_world_data()["types"].get(str(value), {}).get("name"),
                    "quality": "verified_current_ram_type_id"
                    if str(value) in load_world_data()["types"]
                    else "needs_data",
                    "name_quality": "source_prior",
                    "source": "battle_struct type bytes + pinned constants/type_constants.asm",
                }
                for value in type_ids
            ]
            return {
                "nickname": decode_text(self.data(nickname, 11)),
                "species_internal_id": mon[0],
                "species_name_prior": named_lookup("species", mon[0]),
                "level": mon[14],
                "hp": hp,
                ...
                "status_bits": mon[4],
                "types": types,
                "types_verified": types_verified,
                **{name: value if stats_verified[name] else None for name, value in stats.items()},
                "stats_verified": stats_verified,
                "verified_stats": all(stats_verified.values()),
                "stats_quality": "verified_current_ram_battle_stats"
                if all(stats_verified.values())
                else "partially_needs_data",
        ...
        try:
            ...
            party = self.party()
            coherent = any(
                mon["species_internal_id"] == own["species_internal_id"]
                and mon["level"] == own["level"]
                and mon["max_hp"] == own["max_hp"]
                for mon in party
            )
            if not coherent:
            ...
            visible_text = (
                "\n".join(
                    decode_text(
                        tiles[y * 20 + 1 : y * 20 + 19], terminated=False, unknown=" "
                    ).strip()
            ...
            return {
                ...
                "phase": "text_before_combatants_ready" if text_box else "unknown",
                "phase_verified": text_box,
                "menu": "text_or_animation" if text_box else "unknown",
                "visible_text": visible_text,
                "awaiting_input": True if text_box and tiles[358] == 0xEE else None,
        ...
        for attacker, defender in ((own, enemy), (enemy, own)):
            defender_ids = (
                [entry["id"] for entry in defender["types"]] if defender["types_verified"] else []
            )
            for move in attacker["moves"]:
                move_type = move["knowledge"].get("type_id")
                move["effectiveness"] = type_effectiveness(
                    move_type,
                    defender_ids,
                    verified=self._verified_type_chart
                    and move["knowledge"].get("numeric_data_verified") is True
        ...
        menu = (
            "command"
            if all(word in text for word in ("FIGHT", "RUN"))
            and any(word in text for word in ("PACK", "ITEM"))
            else "move"
            if "TYPE/" in text
            or "TYPE" in text
            and any(m["knowledge"].get("name", "").replace("_", " ") in text for m in own["moves"])
            else "text_or_animation"
        )
        ...
        if menu == "move" and pointed:
            for move in own["moves"]:
                ...
                if 1 <= move["move_id"] <= len(names) and pointed == names[
                    move["move_id"] - 1
                ].rstrip("@"):
        ...
        return {
            ...
            "enemy": enemy,
            "verified": True,
            "quality": "verified_coherent_battle_structures",
            "combatants_ready": True,
            "phase": "active",
            "phase_verified": True,
            "type_chart": {
                "verified": self._verified_type_chart,
                "rom_offset": self._type_chart_rom_offset,
            ...
            "menu": menu,
            "menu_cursor_raw": self.byte("wCurrentMenuItem"),
    ...
    def milestones(self, *, party=None, bag=None, world=None, party_ready=True) -> dict:
        ...
        def fact(value, verified, source):
            return {
                "value": value,
                "verified": verified,
                "quality": "verified" if verified else "source_profile_unverified",
                "source": source,
            }

        badge_bits = self.byte("wObtainedBadges")
        result = {
            "has_party": fact(bool(party), True, "validated current party count and structures"),
            "party_count": fact(len(party), True, "validated wPartyCount"),
            "party_move_ids": fact(
                sorted({m["move_id"] for mon in party for m in mon["moves"]}),
                True,
                "validated current party move bytes",
            ),
        ...
        healing_observable = (
            party_ready
            and bool(party)
            and self.byte("wIsInBattle") == 0
            and all(
                mon.get("moves")
                and all(move.get("max_pp_verified") is True for move in mon["moves"])
                for mon in party
            )
        )
        fully_healed = (
            all(
                mon["hp"] == mon["max_hp"]
                and mon["status_bits"] == 0
                and all(move["pp"] == move["max_pp"] for move in mon["moves"])
                for mon in party
            )
            if healing_observable
            else None
        )
        result["party_fully_healed"] = fact(
            fully_healed,
            healing_observable,
        ...
        for bit, badge in enumerate(
            ("boulder", "cascade", "thunder", "rainbow", "soul", "marsh", "volcano", "earth")
        ):
            result["badge_" + badge] = fact(
                bool(badge_bits & 1 << bit), True, "wObtainedBadges bit " + str(bit)
            )
        event_names = {
        ...
        for key, name in {
            "oak_parcel_received": "OAKS_PARCEL",
            "ss_ticket_received": "S_S_TICKET",
            "cut_received": "HM_01",
            "surf_received": "HM_03",
            "strength_received": "HM_04",
            "poke_flute_received": "POKE_FLUTE",
            "silph_scope_received": "SILPH_SCOPE",
            "secret_key_received": "SECRET_KEY",
            "gold_teeth_received": "GOLD_TEETH",
            "lift_key_received": "LIFT_KEY",
            "card_key_received": "CARD_KEY",
        }.items():
            if name in inventory and inventory[name].get("name_verified"):
                result[key] = fact(
                    True,
                    True,
                    "current bag ID and quantity; entire item-name table or HM-range/move-order matched to exact ROM",
                )
            elif key not in result:
                result[key] = fact(
                    True if name in inventory else None,
                    False,
                    "current bag ID matched to unverified pinned-source item name; absence is not nonreceipt",
                )
        result["pokedex_owned_count"] = fact(
            sum(b.bit_count() for b in self.data("wPokedexOwned", 19)),
            False,
            "source-address pokedex bitset, not possession of device",
        )
        ...
        result["hall_of_fame_count"] = fact(
            hof_count,
            counter_layout,
        ...
        result["hall_of_fame_entered"] = fact(
            world.get("name") == "HALL_OF_FAME" and world.get("source_match") is True,
            False,
            "current map identity alone does not prove Hall of Fame registration",
        )
        result["game_completed"] = fact(
            True if self._hof_registered else None,
            self._hof_registered,
    ...
    def _box(tiles: bytes, x: int, y: int, width: int, height: int) -> bool:
        ...
        return (
            at(0, 0) == 0x79
            and at(width - 1, 0) == 0x7B
            and at(0, height - 1) == 0x7D
            and at(width - 1, height - 1) == 0x7E
    ...
    def scene(self) -> dict:
        ...
        if self.byte("wIsInBattle") == 0 and font == 1:
            menu_rows = [
                decode_text(tiles[y * 20 + 12 : y * 20 + 19], terminated=False, unknown=" ").strip()
                for y in range(18)
            ]
            menu_border = any(self._box(tiles, 10, 0, 10, height) for height in (14, 16))
            if (
                "NAME?" in text
                and ("ABCDEFGHI" in compact or "abcdefghi" in compact)
            ...
            if (
                collision
                and tiles[9 * 20 + 8] in collision[1]
                and self.byte("PlayerYPixels") == 60
                and self.byte("PlayerXPixels") == 64
        ...
        if mode == "overworld":
            ...
            if (
                width
                and height
                and not (self.byte("wXCoord") < width and self.byte("wYCoord") < height)
            ):
                mode = "transition"
        return {
            "mode": mode,
    ...
    def dialog(self, scene: dict | None = None) -> dict:
        ...
        if mode == "dialog":
            tiles = self.data("wTileMap", 360)
            lines = [
                decode_text(tiles[y * 20 + 1 : y * 20 + 19], terminated=False, unknown=" ").strip()
                for y in range(13, 17)
            ]
            arrow = tiles[17 * 20 + 18] == 0xEE
            return {
                "open": True,
                "awaiting_input": True if arrow else None,
                "text": "\n".join(line for line in lines if line),
                "choices": ["YES", "NO"]
                if any("YES" in line for line in self.screen_text()["rows"])
                and any("NO" in line for line in self.screen_text()["rows"])
                else [],
                "quality": "verified_text_box_partial_text_possible",
                "source": "RAM wTileMap dialog border/interior and visible down arrow",
                "awaiting_input_reason": "visible_down_arrow"
                if arrow
                else "arrow_absent_or_blinking; printing_or_waiting_unknown",
            }
        known = mode in ("overworld", "main_menu")
        return {
            "open": False if known else None,
            "awaiting_input": False if known else None,
    ...
    def local_map(self, scene: dict | None = None) -> dict | None:
        ...
        rows = [
            "".join(
                "@" if (x, y) == (4, 4) else "." if tile(x, y) in passable else "#"
                for x in range(10)
            )
            for y in range(9)
        ]
    ...
    def snapshot(self) -> dict:
        ...
        def player():
            ...
            facing = (
                FACING.get(self.byte("PlayerFacingDirection"))
                if scene and scene["verified"]
                else None
            )
            return {
                "name": decode_text(self.data("wPlayerName", 11)),
                "map_id": self.byte("wCurMap"),
                "x": self.byte("wXCoord"),
                "y": self.byte("wYCoord"),
                "money": value,
                "badge_bits": self.byte("wObtainedBadges"),
                "facing": facing,
                "facing_source": "RAM PlayerFacingDirection at 0xc109",
        ...
        return {
            ...
            "observation_mode": "read_only_ram",
            "frame": self.emulator.game.frame_count,
            "screen_text": section("screen_text", self.screen_text),
            "player": section("player", player),
            "party": party,
            "bag": bag,
            "party_state": pending.get(
                "party",
                {
                    "ready": party is not None,
            ...
            "milestones": section(
                "milestones",
                lambda: self.milestones(
                    party=party if party is not None else [],
                    bag=bag,
                    world=world,
                    party_ready=party is not None,
                ),
            )
            if (party is not None or "party" in pending) and bag is not None and world is not None
            else None,
            ...
            "scene": scene,
            "dialog": section("dialog", lambda: self.dialog(scene)) if scene else None,
            "local_map": local_map,
            "background_hint": local_map,
            "limitations": [
                "RAM can retain intro/menu/previous-scene values.",
