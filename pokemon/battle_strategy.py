...
def _stat(mon, name):
    verified = (
        mon.get("verified_stats") is True or (mon.get("stats_verified") or {}).get(name) is True
    )
    value = mon.get(name)
...
def _types(mon):
    ...
    for row in values:
        if not isinstance(row, dict) or type(row.get("id")) is not int or row["id"] not in TYPE_IDS:
            return None
        if mon.get("types_verified") is not True and not str(row.get("quality", "")).startswith(
            "verified_"
        ):
            return None
        result.add(row["id"])
...
def _rank_move(move, player, enemy):
    ...
    if knowledge.get("numeric_data_verified") is not True:
        ...
        return excluded("needs_data")
    power, accuracy, attack_type = (
        knowledge.get("power"),
        knowledge.get("accuracy_percent"),
        knowledge.get("type_id"),
    )
    if not (
        _integer(power, 0, 255)
        and _integer(accuracy, 0, 100)
        and type(attack_type) is int
        and attack_type in TYPE_IDS
    ):
    ...
    if factors is None:
        factors = [multiplier]
        row["rounding_note"] = (
            "Only combined type multiplier supplied; intermediate dual-type rounding is approximate."
        )
    elif (
        not isinstance(factors, list)
        or any(not _number(factor, 0, 4) for factor in factors)
    ...
    attack_stat, defense_stat = (
        ("special", "special") if category == "special" else ("attack", "defense")
    )
    ...
    base, damages, scaled_attack, scaled_defense = _damage_distribution(
        player["level"], power, attack, defense, stab, factors
    )
    if not any(damages):
    ...
    ko_probability = (
        sum(damage >= enemy["hp"] for damage in damages) / len(damages) * hit_probability
    )
    row.update(
        eligible=True,
        category=category,
        ...
        defense=defense,
        scaled_attack=scaled_attack,
        scaled_defense=scaled_defense,
        stab=1.5 if stab else 1.0,
        type_factors=factors,
        base_damage=base,
        damage_min=min(damages),
        damage_max=max(damages),
        damage_mean_on_hit=round(mean, 4),
        base_hit_probability=round(hit_probability, 6),
        expected_damage=round(expected, 4),
        expected_effective_damage=round(effective, 4),
...
def plan_battle(observation_or_battle):
    ...
    ranked.sort(
        key=lambda row: (
            row["eligible"],
            row.get("estimated_ko_probability", -1),
            row.get("expected_effective_damage", -1),
            row.get("expected_damage", -1),
            row.get("pp") if type(row.get("pp")) is int else -1,
            -(row["slot"] if type(row["slot"]) is int else 99),
        ),
        reverse=True,
    )
    result["ranked_moves"] = ranked
    result["missing_evidence"] = [
        f"slot {row['slot']}: {missing}" for row in ranked for missing in row["missing_evidence"]
    ]
    candidates = [row for row in ranked if row["eligible"]]
    if not candidates:
        result["status"] = (
            "needs_data" if result["missing_evidence"] else "no_usable_estimated_damage_move"
        )
        return result
    best = candidates[0]
    result["recommended_slot"] = best["slot"]
    result["status"] = "advice_ready"
    result["recommendation_reason"] = (
        "Highest estimated knockout probability, then expected effective damage, among currently usable evaluated moves."
    )
    ...
    if menu == "command":
        ...
        slots = {
            move.get("slot")
            for move in moves
            if isinstance(move, dict) and type(move.get("slot")) is int
        }
        if type(selected) is int and selected in slots:
            result["next_button"] = (
                "a" if selected == best["slot"] else "down" if selected < best["slot"] else "up"
            )
        else:
            result["missing_evidence"].append("visible_normalized_selected_move_slot")
    else:
        result["ui_reason"] = (
            "Damage ranking is retained; text, animation or an unrecognized menu needs current visible UI handling."
        )
    return result
