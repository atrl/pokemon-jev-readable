...
export const requestEvents = (events) =>
  events.filter((event) => event?.type === "jev_request");
export function selectRequest(
  events,
  { selectedId = null, followLatest = true, pinnedEvent = null } = {},
) {
  const requests = requestEvents(events);
  if (followLatest) return requests.at(-1) ?? null;
  return (
    requests.find((event) => event.id === selectedId) ??
...
const childPath = (path, key, isArray = false) =>
  isArray
    ? `${path}[${key}]`
    : /^[A-Za-z_$][\w$]*$/.test(key)
      ? `${path}.${key}`
  ...
  const visit = (current, currentPath, depth) => {
    ...
    if (current !== null && typeof current === "object") {
      for (const [key, child] of Object.entries(current))
        visit(
          child,
          childPath(currentPath, key, Array.isArray(current)),
          depth + 1,
        );
    }
  };
...
function fieldValue(row) {
  if (row.type === "array")
    return row.value.length ? `[ ${row.value.length} items ]` : "[]";
  if (row.type === "object") {
...
function block(title, value, className = "") {
  ...
  section.append(
    element("h4", "", title),
    element("pre", "input-text", readable(value)),
  );
  return section;
}
...
export function createRequestInspector(root) {
  ...
  function renderFields() {
    ...
    const matching = flattened.filter(
      (row) =>
        !search ||
        `${row.path} ${row.type} ${fieldValue(row)}`
          .toLocaleLowerCase()
          .includes(search),
    );
    ...
    $("field-count").textContent =
      `显示 ${matching.length} / ${flattened.length} 项（含容器）`;
      ...
      for (const [questionId, question] of Object.entries(questions)) {
        ...
        card.append(
          element(
            ...
            `问题 ${questionId} · type: ${scalar(question?.type)}`,
          ),
        );
        card.append(
          block("instructions · 完整提示词原文", question?.instructions),
        );
        const criteria = question?.criteria;
        const candidates = element("details", "input-candidates");
        candidates.open = true;
        candidates.append(
          element("summary", "", "criteria · 候选 ID 与完整描述原文"),
        );
        if (
          criteria !== null &&
          ...
          candidates.append(element("pre", "input-text", readable(criteria)));
        ...
        if (Object.keys(other).length)
          card.append(block("该问题的其他原始字段", other));
  ...
  function renderState(request) {
    ...
    const keys = [
      ...new Set([
        ...
        ).filter((key) => !["goal", "current_focus"].includes(key)),
      ]),
    ];
    for (const key of keys) {
      ...
      const label = !exists
        ? "未提供（missing）"
        : kind === "object"
          ? `object · ${Object.keys(value).length} 个字段`
          : kind === "array"
    ...
    $("unavailable").replaceChildren(
      block(
        "state.game.unavailable · 请求声明的信息缺口",
        state?.game?.unavailable,
      ),
    );
    ...
    const fragment = document.createDocumentFragment();
    $("transition-count").textContent = Array.isArray(transitions)
      ? `${transitions.length} 组实际记录`
      : "未提供转换列表";
    if (Array.isArray(transitions) && transitions.length) {
      fragment.append(
        block(
          "temporal_context · 列表顺序与含义原文",
          Object.fromEntries(
            Object.entries(temporal).filter(([key]) => key !== "transitions"),
          ),
        ),
      );
      ...
      transitions.forEach((transition, index) => {
        ...
        card.append(
          element(
            "h4",
            "",
            `记录 ${index + 1} · step ${scalar(transition?.step)} · button ${scalar(transition?.button)}`,
          ),
        );
        card.append(
          block("before · 按键前", transition?.before),
          block("after · 按键后", transition?.after),
          block("outcome · 已记录效果", transition?.outcome),
        );
        cards.append(card);
      });
  ...
  function renderSelected() {
    ...
    $("source").textContent =
      `model: ${scalar(request?.model)} · ${eventLabel(selected)} · event: ${selected.id}`;
    $("goals").replaceChildren(
      block("state.goal · 总目标原文", request?.state?.goal),
      block(
        "state.current_focus · 当前重点原文",
        request?.state?.current_focus,
      ),
    );
  ...
  function renderSelection() {
    const retained =
      selected && requests.some((event) => event.id === selected.id);
    ...
    if (picker.dataset.signature !== signature) {
      ...
      for (const event of options) {
        const option = element(
          "option",
          "",
          `${eventLabel(event)}${event === selected && !retained ? " · 已固定，超出窗口" : ""}`,
        );
        option.value = event.id;
        fragment.append(option);
      }
      if (!options.length)
        fragment.append(element("option", "", "尚无真实请求"));
    ...
    follow.textContent = followLatest
      ? "跟随最新请求 · 点击固定"
      : "已固定此请求 · 恢复跟随";
    ...
    $("window").textContent =
      `历史选择来自最近最多 2,000 条事件窗口，当前窗口含 ${requests.length} 次请求。${selected && !retained ? "此固定请求已超出窗口，当前页面仍保留这一份。" : ""}选择历史请求会固定阅读；较早的本机日志不在此选择器内。`;
  ...
  follow.addEventListener("click", () => {
    ...
    selected = selectRequest(requests, {
  ...
  $("copy").addEventListener("click", async () => {
    ...
    try {
      ...
      $("status").textContent =
        "浏览器未允许直接复制；完整 JSON 已选中，可手动复制。";
    }
  });
  $("download").addEventListener("click", () => {
    if (!selected || !own(selected, "request")) return;
    const url = URL.createObjectURL(
      new Blob([json(selected.request)], { type: "application/json" }),
    );
    const link = element("a");
    link.href = url;
    link.download =
      `${runId}-step-${selected.step ?? "unknown"}-attempt-${selected.attempt ?? "unknown"}.json`.replace(
        /[^\w.-]/g,
        "_",
      );
    link.click();
