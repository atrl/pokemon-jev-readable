...
class CoreTests(unittest.TestCase):
    ...
    def test_unverified_fields_never_enter_model_state(self):
        raw = {
            "player": {"name": "RED", "x": 3, "y": 6, "map_id": 38, "money": 3000},
            "party": [{"species_internal_id": 176}],
            "bag": [{"item_id": 20}],
            "battle_type_raw": 2,
            "background_hint": {"rows": ["...."]},
        }
        model = observation_for_model(raw)
        self.assertNotIn("money", model["player"])
        self.assertIsNone(model["party"])
        self.assertIsNone(model["bag"])
    ...
    def test_valid_answer(self):
        self.assertEqual(validate_response(self.answer())["choice"], "a")

    def test_unknown_action_rejected(self):
        a = self.answer()
        a["answers"]["button"]["choice"] = "go_to_lab"
        with self.assertRaises(ValueError):
            validate_response(a)

    def test_nonfinite_rejected(self):
        a = self.answer()
        a["answers"]["button"]["probabilities"]["a"] = math.nan
        with self.assertRaises(ValueError):
            validate_response(a)

    def test_missing_option_rejected(self):
        a = self.answer()
        del a["answers"]["button"]["probabilities"]["wait"]
        with self.assertRaises(ValueError):
            validate_response(a)
    ...
    def test_nonempty_party_fixture(self):
        ...
        b = a["wPartyMon1"]
        m.memory[b] = 176
        m.memory[b + 1 : b + 3] = (18).to_bytes(2, "big")
        m.memory[b + 33] = 5
        m.memory[b + 34 : b + 36] = (20).to_bytes(2, "big")
        m.memory[b + 8] = 10
        m.memory[b + 29] = 35
        ...
        self.assertEqual(
            [{key: move[key] for key in ("move_id", "pp")} for move in mon["moves"]],
            [{"move_id": 10, "pp": 35}],
        )
    ...
    def test_invalid_party_count_rejected(self):
        m = FakeMemory()
        p = load_profile()
        m.memory[p["addresses"]["wPartyCount"]] = 7
        with self.assertRaises(ValueError):
            Reader(m, p).party()
    ...
    def test_wrong_rom_rejected_before_import(self):
        with tempfile.TemporaryDirectory() as d:
            f = Path(d) / "wrong.gb"
            f.write_bytes(b"not a rom")
            with self.assertRaises(ValueError):
                Emulator(f, load_profile())
    ...
    def test_ci_reports_missing_key_instead_of_faking_play(self):
        with tempfile.TemporaryDirectory() as d, patch.dict("os.environ", {}, clear=True):
            result = run(
                Path("absent.gb"), Path(d) / "run", goal="Explore", steps=12, allow_missing_key=True
            )
