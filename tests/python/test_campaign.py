...
def state(x=3, y=6, mid=38):
    return {
        "player": {"map_id": mid, "x": x, "y": y, "facing": "up"},
        ...
        "world": {
            ...
            "objects": [],
            "warps": [
                {"x": 7, "y": 1, "destination_map_id": 37, "quality": "verified_current_ram"}
            ],
            "connections": [],
        },
        "local_map": {
...
def target(map_id=38, kind="coordinate", x=5, y=6):
    return {
        ...
        "interaction_selectors": [{"kind": kind, "x": x, "y": y}],
        "completion": False,
    }
...
class CampaignTests(unittest.TestCase):
        ...
        s["world"]["objects"] = [
            {
                "x": 6,
                "y": 3,
                "kind": "npc_or_interactive",
                "sprite": "BALL",
                "text_id": 2,
                "active": True,
            }
        ]
        objective = target(kind="object", x=6, y=3)
        objective["interaction_selectors"][0].update(sprite="BALL", text_id=2)
    ...
    def test_border_warp_requires_outward_activation(self):
        s = state(x=2, y=7, mid=37)
        s["world"]["warps"] = [{"x": 2, "y": 7, "destination_map_id": 0}]
    ...
    def test_transition_and_clues_persist_longer_than_short_action_window(self):
        ...
        a = state(7, 2)
        b = state(7, 1, 37)
        planner.record("up", a, b)
        for _ in range(40):
            planner.record("wait", b, b)
        restored = CampaignPlanner(
            json.loads(json.dumps(planner.snapshot())), selector=lambda *_: target()
        )
    ...
    def test_unverified_and_attempt_local_facts_are_not_persisted_as_story_completion(self):
        ...
        s["milestones"] = {
            ...
            "elite4_lance_defeated": {"value": True, "verified": True},
            "hall_of_fame_entered": {"value": True, "verified": False},
        }
        p = CampaignPlanner(selector=lambda *_: target())
        p.context(s)
        self.assertEqual(set(p.history_facts), {"pokedex_received"})

    def test_unknown_map_alignment_does_not_produce_a_direction(self):
        s = state()
        s["world"]["source_match"] = False
        p = CampaignPlanner(selector=lambda *_: target())
        n = p.context(s)["navigation"]
        self.assertEqual(n["status"], "wait_for_map_transition")
        self.assertEqual(n["next_button"], "wait")
    ...
    def test_hidden_target_is_not_reintroduced_from_source_coordinates(self):
        s = state()
        s["world"]["objects"] = [{"x": 6, "y": 3, "sprite": "BALL", "text_id": 2, "active": False}]
        objective = target(kind="object", x=6, y=3)
        objective["interaction_selectors"][0].update(sprite="BALL", text_id=2)
    ...
    def test_turn_and_temporary_failed_edge_do_not_become_permanent_walls(self):
        ...
        a = state()
        b = state()
        b["player"]["facing"] = "right"
        p.record("right", a, b)
        self.assertFalse(p.failed_edges)
        p.record("right", b, b)
        p.record("right", b, b)
        self.assertTrue(p.failed_edges)
        restored = CampaignPlanner(p.snapshot(), selector=lambda *_: target())
        for _ in range(65):
            restored.record("wait", b, b)
        self.assertFalse(restored.failed_edges)

    def test_starter_received_guidance_heads_to_rival_trigger_not_rival_text(self):
        s = state(x=5, y=3, mid=40)
        s["world"].update(width=10, height=12)
        s["milestones"] = {
            "starter_received": {"value": True, "verified": True},
            "party_count": {"value": 1, "verified": True},
        }
        s["world"]["objects"] = [
            {"x": 4, "y": 3, "sprite": "SPRITE_BLUE", "text_id": 1, "active": True}
        ]
    ...
    def test_scripted_warp_does_not_publish_invalid_tiles_or_replayable_edges(self):
        p = CampaignPlanner()
        a = state(x=11, y=12, mid=0)
        a["world"]["input_lock"] = {"ignored_buttons_mask": 252, "scripted_movement_remaining": 2}
        b = state(x=12, y=11, mid=40)
        b["world"].update(source_match=False, player_position_valid=False)
        p.record("right", a, b)
        self.assertNotIn("40", p.tiles)
        self.assertFalse(p.transitions)
        b = state(x=5, y=11, mid=40)
        b["world"].update(height=12, width=10)
        p.record("right", a, b)
        self.assertEqual(p.transitions[-1]["kind"], "script_or_battle")
    ...
    def test_new_dialogue_counts_as_ui_progress_but_repeated_dialogue_does_not(self):
        ...
        b = state()
        b["scene"]["mode"] = "dialog"
        b["dialog"] = {"open": True, "text": "A new story explanation"}
    ...
    def test_nurse_is_interactable_from_the_public_side_of_counter(self):
        s = state(x=3, y=3, mid=41)
        s["world"].update(width=14, height=8)
        s["world"]["objects"] = [
            {"x": 3, "y": 1, "sprite": "SPRITE_NURSE", "text_id": 1, "active": True}
        ]
    ...
    def test_legacy_checkpoint_without_optional_memories_keeps_dialog_deduplication(self):
        planner = CampaignPlanner(selector=lambda *_: target())
        closed = state()
        opened = state()
        opened["scene"]["mode"] = "dialog"
        opened["dialog"] = {"open": True, "text": "An existing saved clue"}
        planner.record("a", closed, opened)
        checkpoint = json.loads(json.dumps(planner.snapshot()))
        for optional in ("support", "recovery", "seen_clues", "failed_edge_steps"):
            checkpoint.pop(optional)

        restored = CampaignPlanner(checkpoint, selector=lambda *_: target())
        self.assertFalse(restored.record("wait", opened, opened)["new_dialog_clue"])
        self.assertEqual(restored.clues, planner.clues)
        self.assertIsNone(restored.support)
        self.assertIsNone(restored.recovery)

    def test_saved_recovery_guidance_survives_restore_and_clears_after_scene_change(self):
        planner = CampaignPlanner(selector=lambda *_: target())
        overworld = state()
        planner.context(overworld)
        planner.recover(overworld, attempt=2, reason="repeated_input", failed_button="right")
        checkpoint = json.loads(json.dumps(planner.snapshot()))
        restored = CampaignPlanner(checkpoint, selector=lambda *_: target())
        self.assertEqual(restored.context(overworld)["recovery"], planner.recovery)

        dialog = state()
        dialog["scene"]["mode"] = "dialog"
        dialog["dialog"] = {"open": True, "text": "An interaction started"}
        self.assertIsNone(restored.context(dialog)["recovery"])
        self.assertEqual(restored.active["id"], planner.active["id"])
