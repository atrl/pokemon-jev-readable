...
if __name__ == '__main__':
    ...
    parser.add_argument('--output', type=Path, default=ROOT / "outputs/tests/dual")
    parser.add_argument('--steps', type=int, default=80)
    parser.add_argument('--allow-model-calls', action='store_true', help='Explicitly allow this bounded test to spend API credits')
    args = parser.parse_args()
    if not args.allow_model_calls:
        parser.error('Real model calls require --allow-model-calls; no API request was made')
    if not 1 <= args.steps <= 80:
        parser.error('smoke steps must be 1..80')
    result = verify(args.rom, args.state, args.output, args.steps)
    if not result['dual_model_integration_observed']:
        raise SystemExit(1)
