...
def verify(rom: Path, output: Path, checkpoint: Path) -> dict:
    ...
    report = {
        ...
        "policy": "scripted_regression_not_jev",
        "checks": {},
        "jev_called": False,
    }
    history = []
    try:
        ...
        for i in range(120):
            ...
            if (
                reader.byte("wCurMap") == 38
                and reader.data("wTileMap")[0] == 16
                and reader.byte("wFontLoaded") == 0
                and reader.byte("wTilesetBank") > 0
            ):
                break
            if "NEW NAME" in text:
            ...
            raise AssertionError(
                "Test fixture did not reach the starting room; do not report a pass"
            )
        world.tick(60)
        ...
        report["checks"]["menu_cursor"] = {
            "before": before_cursor,
            "after": after_menu["menu_cursor_raw"],
            "visible_menu": True,
        }
        world.load(baseline)
        ...
        write_json(
            ...
            {
                "rom_sha1": profile["rom_sha1"],
                "state_sha256": hashlib.sha256(baseline).hexdigest(),
                "source": "scripted regression fixture; not Jev gameplay",
            },
        )
        report["checks"].update(
            name="RED", dialog_decoded=seen_dialog, empty_party_and_bag=True, state_replay=True
        )
        assert seen_dialog
        report.update(
            status="passed",
...
if __name__ == "__main__":
    ...
    ap.add_argument("--output", type=Path, default=ROOT / "outputs/tests/rom")
