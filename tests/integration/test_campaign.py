...
def main():
    ...
    assert (
        ...
        == "378853bf409dedb3ea85acc0c5c3df5e24195d500633a560ec8bee0b004283b9"
    )
    ...
    b = copy.read_bytes()
    e = Emulator(args.rom, load_profile())
    e.load(b)
    r = Reader(e, load_profile())
    report = {
        ...
        "ram_writes": 0,
        "screenshots": 0,
        "policy": "isolated_scripted_regression_not_jev_or_production",
        "created_at": datetime.now(timezone.utc).isoformat(),
        "rom_sha1": load_profile()["rom_sha1"],
        "source_commit": load_profile()["source_commit"],
        "source_binary_match": False,
        "terminal_validation": "ROM counter instruction confirmed; live Hall of Fame registration NOT reached",
        "checks": {},
    }
    ...
    def save(name):
        ...
        report["checks"][name]["screen_text_nonblank"] = [
            line for line in s["screen_text"]["rows"] if line
        ]
        ...
        for _ in range(limit):
            ...
            if s["scene"]["mode"] != "overworld":
                if s["battle"]["active"]:
                    for _ in range(150):
                        if (
                            snap()["battle"].get("menu") == "command"
                            and "rival_battle_live" not in report["checks"]
                        ):
                            save("rival_battle_live")
                        press("a")
                        if not snap()["battle"]["active"]:
                            break
                    continue
                press("a")
                continue
            grid = s["local_map"]["rows"]
            cells = known.setdefault(mid, {})
            for y, row in enumerate(grid):
                for x, char in enumerate(row):
                    pos = (start[0] + x - 4, start[1] + y - 4)
                    if 0 <= pos[0] < s["world"]["width"] and 0 <= pos[1] < s["world"]["height"]:
                        cells[pos] = char != "#"
            occupied = {(o["x"], o["y"]) for o in s["world"]["objects"] if o["present"] is True}
            blocked_warps = {
                (w["x"], w["y"]) for w in s["world"]["warps"] if (w["x"], w["y"]) != target
            }
            ...
            while queue:
                ...
                for k, (dx, dy) in delta.items():
                    ...
                    if (
                        ...
                        and nxt not in blocked_warps
                    ):
                        seen.add(nxt)
                        queue.append((nxt, path + [k]))
            if best is None:
                # Reach the observed frontier closest to target to expose more map.
                candidates = [p for p in seen if p != start]
                candidate = (
                    min(candidates, key=lambda p: abs(p[0] - target[0]) + abs(p[1] - target[1]))
                    if candidates
                    else None
                )
                if candidate is None:
                    raise RuntimeError(("no route", start, target, s["world"]))
                queue = collections.deque([(start, [])])
                seen = {start}
                while queue:
                    pos, path = queue.popleft()
                    ...
                    for k, (dx, dy) in delta.items():
                        ...
                        if (
                            ...
                            queue.append((nxt, path + [k]))
            if not best:
                raise RuntimeError(("empty path", start, target))
            press(best[0])
            a = snap()["player"]
            if (a["map_id"], a["x"], a["y"]) == (mid, *start):
                dx, dy = delta[best[0]]
                cells[(start[0] + dx, start[1] + dy)] = False
    ...
    try:
        ...
        if snap()["player"]["map_id"] == 0:
            ...
            for _ in range(70):
                press("a")
                if snap()["player"]["map_id"] == 40 and snap()["scene"]["mode"] == "overworld":
                    break
            dialog_close()
            save("oak_lab")
        if snap()["player"]["map_id"] == 40 and not snap()["party"]:
            ...
            for _ in range(25):
                press("a")
                if snap()["battle"].get("verified") and snap()["battle"].get("menu") == "command":
                    break
            save("rival_battle")
            for _ in range(120):
                press("a")
                if (
                    not snap()["battle"]["active"]
                    and snap()["milestones"]["rival_lab_battled"]["value"]
                ):
                    break
            save("after_rival")
            dialog_close()
            ...
            go(40, (4, 11))
            press("down")
            save("pallet_after_lab")
            go(0, (10, 0))
            press("up")
            save("route1")
            go(12, (10, 0), limit=300)
            press("up")
            save("viridian")
            go(1, (29, 19), limit=150)
            save("mart_entered")
            for _ in range(50):
                press("a")
                if snap()["milestones"]["oak_parcel_received"]["value"]:
                    break
            save("parcel_received")
            dialog_close()
            go(42, (3, 7))
            press("down")
            save("viridian_return")
            go(1, (20, 35))
            press("down")
            save("route1_return")
            go(12, (10, 35), limit=300)
            press("down")
            save("pallet_return")
            go(0, (12, 11))
            save("lab_return")
            ...
            for _ in range(60):
                press("a")
                if snap()["milestones"]["pokedex_received"]["value"]:
                    break
        ...
        e.close()
        temporary.cleanup()
        assert hashlib.sha256(state.read_bytes()).hexdigest() == report["input_sha256"]
    ...
    print(
        "Verified isolated map, hidden sprites, control lock, starter, rival, parcel and Pokedex observations; production state unchanged."
    )
